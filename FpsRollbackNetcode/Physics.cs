...
namespace FpsRollbackNetcode
{
    ...
    public static class Physics
    {
        ...
        public static StaticHandle AddStaticBox( Vector3 position, Quaternion rotation, float width, float height, float length)
        {
            var box = new Box(width, height, length);
            var boxCollidable = Simulation.Shapes.Add(box);
            //var rotation = Quaternion.Identity;
            var boxPose = new RigidPose(position, rotation);
            var boxDescription = new StaticDescription(boxPose, boxCollidable);
            return Simulation.Statics.Add(boxDescription);
        }

        public static BodyHandle AddBall(Vector3 position, float radius = 1f)
        {
            //Drop a ball on a big static box.
            var sphere = new Sphere(radius);
            var sphereInertia = sphere.ComputeInertia(1);
            var sphereCollidable = Simulation.Shapes.Add(sphere);
            var sphereDescription = BodyDescription.CreateDynamic(position, sphereInertia, sphereCollidable, new BodyActivityDescription(0.01f));
            return Simulation.Bodies.Add(sphereDescription);
        }

        public static void Simulate(float deltaTime)
        {
            Simulation.Timestep(deltaTime*0.001f);
        }

        // public static void Run()
        // {
        //     Console.WriteLine(_simulation.Bodies[_sphereBodyHandle].Pose.Position.Y);
        //     //Now take 100 time steps!
        //     var i = 0;
        //     for (var o = 0; o < 2; o++)
        //     {
        //         (MotionState MotionState, SolverState SolverState, bool Awake) thing = default;
        //         for (; i < 100; ++i)
        //         {
        ...
        //
        //             var innerSphereSimulationBody = _simulation.Bodies[_sphereBodyHandle];
        //             if (i == 6)
        //             {
        //                 thing = (innerSphereSimulationBody.MotionState,
        //                     innerSphereSimulationBody.SolverState,
        //                     innerSphereSimulationBody.Awake);
        //
        //
        //                 Console.WriteLine("Save!");
        //             }
        //
        //             _simulation.Timestep(0.1f);
        //
        //             Console.WriteLine(_simulation.Bodies[_sphereBodyHandle].Pose.Position.Y);
        //         }
        //         Console.WriteLine("Reset!");
        //
        //         var sphereSimulationBody = _simulation.Bodies[_sphereBodyHandle];
        //         sphereSimulationBody.MotionState = thing.MotionState;
        //         sphereSimulationBody.SolverState = thing.SolverState;
        //         sphereSimulationBody.Awake = thing.Awake;
        //         i = 6;
        //     }
        //     
        // }
